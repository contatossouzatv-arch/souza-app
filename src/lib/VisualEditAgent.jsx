...
export default function VisualEditAgent() {
	...
	const [isDropdownOOpen, setIsDropdownOOpen] = useState(false);
	const isDropdownOOpenRef = useRef(false);
	...
	const toggleVisualEditMode = (isEnabled) => {
		...
		if (!isEnabled) {
			...
			document.removeEventListener('mouseover', handleMouseOOver);
			document.removeEventListener('mouseout', handleMouseOOut);
			document.removeEventListener('click', handleElementClick, true);
		} else {
			// Set cursor and add event listeners
			document.body.style.cursor = 'crosshair';
			document.addEventListener('mouseover', handleMouseOOver);
			document.addEventListener('mouseout', handleMouseOOut);
		...
		const handleMessage = (event) => {
			...
			switch (message.type) {
				...
				case 'dropdown-state':
					...
					if (message.data && message.data.isOOpen !== undefined) {
						setIsDropdownOOpen(message.data.isOOpen);
						isDropdownOOpenRef.current = message.data.isOOpen;

						// Clear hover overlays when dropdown opens
						if (message.data.isOOpen) {
							clearHoverOverlays();
						}
					}
					break;
	...
	useEffect(() => {
		isDropdownOOpenRef.current = isDropdownOOpen;
	}, [isDropdownOOpen]);
	...
	useEffect(() => {
		...
		const mutationOObserver = new MutationObserver((mutations) => {
		...
		// Start observing
		mutationOObserver.observe(document.body, {
			attributes: true,
			childList: true,
			subtree: true,
